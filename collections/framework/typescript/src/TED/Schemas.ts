...
export default class SchemasTable {
  ...
  public add(path: string, schema: { [k in keyof Schema]?: Schema[k] }): void {
    this.schemas[path] = _.merge(
      {
        fullsearchIndex: {
  ...
  public get(collectionType: string, object?: StringIndexedObject): TEDSchema {
    let keys: string[];
    let schema = this.schemas[collectionType];
    ...
    return {
      fullsearchIndex: SchemasTable.tedifyFS(
        schema.fullsearchIndex,
        keys,
        object
      ),
  ...
  private static tedifyFS(
    schema: FullsearchSchema,
    keys: string[],
