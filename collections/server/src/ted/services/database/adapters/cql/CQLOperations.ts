...
export class CQLSaveOperation extends CQLBaseOperation {
  ...
  public async execute(): Promise<myTypes.ServerAnswer> {
    ...
    return await super.execute().then((result) => {
      timer.stop();
      return result;
    });
  }
...
export class CQLGetOperation extends CQLBaseOperation {
  ...
  protected buildQuery(): void {
    ...
    if (this.options.order !== undefined) {
      ...
      const order_key = this.options.order.key;
      const order_direction = this.options.order.order;

      Object.entries(this.entry)
        .slice(1)
        .forEach(([key, value]) => {
          res.push(key);
          res.push(order_direction);
          res.push(",");
          params.push(value);
        });
...
export class CQLBatchOperation implements myTypes.DBDirectOperation {
  ...
  protected buildQueries(): void {
    ...
    for (let op of this.operations) {
      if (op.query === null)
        throw new Error("Error in batch, a base query is not built");
  ...
  constructor(batch: CQLBaseOperation[]) {
    ...
    for (let op of this.operations) {
      if (
        ...
        throw new Error("Array cannot contain batch or get operations");
    }
    this.queries = null;
  }
