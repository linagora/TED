...
export function setup() {
  const password: Buffer = Buffer.from(
    config.configuration.crypto.password,
    "hex"
  );
  const salt: Buffer = Buffer.from(config.configuration.crypto.salt, "hex");
  globalKey = crypto.createSecretKey(
    crypto.pbkdf2Sync(
      password,
      salt,
      1000000,
      config.configuration.crypto.keyLen,
      "sha512"
    )
  );
}
...
export function decryptResult(
  ans: myTypes.ServerAnswer,
  key: crypto.KeyObject
): void {
  if (
    ans.queryResults !== undefined &&
    ...
    for (let i: number = 0; i < ans.queryResults.allResultsEnc.length; i++) {
      let encObject: myTypes.EncObject = JSON.parse(
        ans.queryResults.allResultsEnc[i].object
      );
      let clearData: myTypes.ServerSideObject = decryptData(encObject, key);
      let objectIDs: myTypes.ServerSideObject =
        ans.queryResults.allResultsEnc[i];
...
export function encryptOperation(
  ...
  if (
    operation.action === myTypes.action.save &&
    operation.clearObject !== undefined
  )
    operation.encObject = JSON.stringify(
      encryptData(operation.clearObject, key)
    );
  else if (
...
function encryptData(
  ...
  const cipher: crypto.Cipher = crypto.createCipheriv(
    config.configuration.crypto.algorithm,
    key,
    iv
  );
...
export function decryptData(
  encObject: myTypes.EncObject,
  key: crypto.KeyObject
): myTypes.ServerSideObject {
  globalCounter.inc("decryption");
  let timer = new Timer("decryption");
  if (encObject.iv === undefined)
    throw new Error("Unable to decrypt data, missing iv");
  if (encObject.auth === undefined)
    throw new Error("Unable to decrypt data, missing auth");
  const decipher: crypto.Decipher = crypto.createDecipheriv(
    config.configuration.crypto.algorithm,
    key,
    Buffer.from(encObject.iv, "base64")
  );
...
function isDecipherGCM(
  decipher: crypto.Decipher
): decipher is crypto.DecipherGCM {
