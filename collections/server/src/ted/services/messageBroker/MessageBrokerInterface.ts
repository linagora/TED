...
export class RabbitMQBroker extends TaskBroker {
  ...
  public async runTasks(): Promise<void> {
    try {
      ...
      await Promise.race([
        this.channel.prefetch(this.prefetechCount),
        ineterruptPromise,
      ]);
      if (!this.running) throw interruptionError;
...
export class SQSBroker extends TaskBroker {
  ...
  constructor(
    callback: (path: string) => Promise<void>,
    queueOptions: SQS.CreateQueueRequest,
    messageOptions: Object,
    prefetechCount: number,
    fifo: boolean
  ) {
    super(callback, queueOptions, messageOptions);
    this.sqs = new SQS({
      region: config.configuration.sqs.region,
      accessKeyId: config.configuration.sqs.accessID,
      secretAccessKey: config.configuration.sqs.accessKey,
    });
    this.fifo = fifo;
        ...
        this.sqs.receiveMessage(
          ...
          async function (err, data) {
            try {
              ...
              if (!that.running) throw interruptionError;
      ...
      try {
        if (this.queueURL === undefined) {
          ...
          this.sqs.createQueue(
            this.queueOptions as SQS.CreateQueueRequest,
            function (err, data): void {
              if (err) throw err;
